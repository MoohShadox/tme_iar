...
def ddpg(n_episodes=600, max_t=200, print_every=100):
...
def td3(n_episodes=600, max_t=200, print_every=100):
    scores_deque = deque(maxlen=print_every)
    agent = TD3_Agent(state_size=3, action_size=1, random_seed=2)
    scores = []
    for i_episode in range(1, n_episodes + 1):
        state = env.reset()
        agent.reset()
